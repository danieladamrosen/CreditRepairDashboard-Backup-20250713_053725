    ...
    <div data-section="negative-accounts">
      <Card className={cn(
        ...
        negativeAccountsCollapsed
          ? (allNegativeAccountsSaved ? "border-2 border-green-500 bg-green-50" : "border-2 border-red-500 bg-rose-50")
        ...
        <CardHeader 
          ...
          className={cn(
            ...
            negativeAccountsCollapsed
              ? (allNegativeAccountsSaved ? "hover:bg-green-100" : "hover:bg-red-100")
              : "hover:bg-gray-50"
          )}
        >
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full text-white text-sm font-bold flex items-center justify-center ${allNegativeAccountsSaved ? 'bg-green-500' : 'bg-red-500'}`}>
            ...
            <div>
              <h3 className={`text-lg font-bold m-0 ${allNegativeAccountsSaved ? 'text-green-700' : 'text-red-700'}`}>
                {allNegativeAccountsSaved ? 'Negative Accounts – Disputes Saved' : 'Negative Accounts'}
              </h3>
              <p className={`flex flex-row items-center gap-2 text-sm m-0 ${allNegativeAccountsSaved ? 'text-green-600' : 'text-red-600'}`}>
                {allNegativeAccountsSaved ? (
          ...
          <div className="flex flex-row items-center gap-2">
            <span className={`text-sm ${allNegativeAccountsSaved ? 'text-green-600' : 'text-red-600'}`}>{negativeAccounts.length} accounts</span>
            {negativeAccountsCollapsed ? (
              <ChevronDown className={`h-5 w-5 ${allNegativeAccountsSaved ? 'text-green-600' : 'text-red-600'}`} />
            ) : (
              <ChevronUp className={`h-5 w-5 ${allNegativeAccountsSaved ? 'text-green-600' : 'text-red-600'}`} />
            )}
          </div>
        </CardHeader>
